...
console.log(splitMe('oggi è lunedì'));
...
console.log(deleteOne('Oggi è martedì', false));
...
function whatDayIsIt() {
  ...
  const giorno = dataOggi.getDay();
  let nomeGiorno = ['domenica', 'lunedì', 'martedì', 'mercoledì', 'giovedì', 'venerdì', 'sabato'  ]
 
  return nomeGiorno[giorno];
};
...
function isTodayMyBirthday (){
  let oggi = new Date();
  let bday = "31/12";
  let controllo = oggi.getDate() + '/' +  oggi.getMonth() +1 ;
  if (controllo === bday){
    return true;
  } else return false;
}
...
function onlyInLastMillennium(){
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    const filtro = listaAnni.filter((el) => el < 2000);
    return filtro;
